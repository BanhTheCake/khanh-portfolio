    ...
    <div className="relative flex flex-col items-center w-full">
            ...
            {Images && Images.map((image, index) => (
                <div className={`flex flex-col md:flex-row ${image.isCenter ? "md:items-center" : "md:items-end"} justify-center py-5 md:py-10 gap-10  md:gap-20 ${image.position === "right" ? "md:flex-row-reverse" : ""} z-1`} key={index}>
                    <motion.div animate={{ y: [0, -30, 0] }} transition={{ duration: 5, repeat: Infinity, ease: "linear", delay: image.delay }}  className={`flex-3 flex`}>
                    ...
                    transition={{ duration: 1, ease: "linear"  }} className={`flex-2 flex flex-col justify-center px-5 md:px-0`}>
